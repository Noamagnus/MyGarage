...
class GarageService {
  ...
  // todo kill stream if not needed anymore
  Stream<List<Car>> getAllCarsSteams() {
   ...
   Future<List<Car>> getAllCarsFuture() async {
    final finder = Finder(sortOrders: [
      SortOrder('id'),
    ]);
    final recordSnapshot = await _store.find(_db.instance, finder: finder);

    return recordSnapshot.map((snapshot) {
      final exercise = Car.fromJson(snapshot.value);
      print('Key is ${snapshot.key}');
      // car.uuid = snapshot.key;
      // car.id == snapshot.key;
      return exercise;
    }).toList();
  }
